...
The relationship between these quantities is described by the \emph{spectral efficiency function}. It states how efficiently a human eye reacts to different wavelengths, i.e. we can detect some wavelengths more easily than others. As we can see in \ref{fig:lum}, the scotopic (night) perception peaks at around 507nm and the photopic (day) at 555nm.
...
An image interpretation of the function is in \ref{fig:brdf}. As it is a distribution function, we can also reformulate it's meaning as a probability density that a defined amount of light energy gets reflected from $\omega_i$ to $\omega_o$.
...
\begin{enumerate}
	\item Compute the tristumulus value XYZ using the CIE color matching functions (shown in \ref{fig:cmf})
	\begin{align*} 
	...
	\item Convert the XYZ to the desired RGB color space using a transformation matrix. Depending on the specific RGB color space, the matrix differs --- an example of CIE XYZ to sRGB conversion is demonstrated in \ref{eq:conversion}.
	\begin{align*}  
...
The spectral rendering presents the ability to reproduce the colors in a more photorealistic way. We might not see it at first, but the colors produced by a conventional RGB renderer tend to be more colorful as they do not account for the spectral characteristics. A comparison between the RGB and the spectral render of the same scene by Mitsuba2 is shown in \ref{fig:compare_color}.
...
The biggest advantage are natural phenomena for which the tracing of multiple wavelengths is an absolute necessity. Namely, those are:
\begin{description}
	\item[Fluorescence] Re-emission of colors under different lights
	\item[Dispersion] Refraction and splitting of a white light into its wavelength components
...
\end{description}
They are explained in detail in \ref{chap:appearance}.

\subsection{Disadvantages}

On the other side, there is a need to track multiple wavelengths at once which requires a numerical integration over them. This introduces a problem of chromatic noise. Moreover, while the performance is not a key factor, it obviously worsens as well. 

Other than the tracking issues,  we are still unable to properly display spectral images on the current monitors. The only option here would to display separate channels of an EXR image where each represents one spectral band. This is quite inconvenient as the result is not one image but multiple. Therefore, the rendering needs to have a well-done spectrum to RGB conversion as the final image is still being displayed on a RGB monitor. Fortunately, the conversion is well-defined and should not pose a challenge for anyone who could create a spectral renderer.

Due to the larger size of the visible spectrum compared to the RGB gamut, the situation gets significantly more complicated for the reversed conversion from the tristimulus RGB to the spectral domain --- called the \emph{spectral upsampling}, as there exist infinitely many spectra for a single RGB triplet. For those who may be interested in the details of the current development to the spectral upsampling, refer to the article by \citet{jakob2019low} which proposes a solution that converts full sRGB gamut with zero error. 

There exist several reasons to provide the spectral upsampling, mainly because the spectral values are a lot harder to determine as you need a specific device (spectrometer) that would measure the color values under a specific light and then use regularly distributed samples from it as an input. Because of the reproducibility of the RGB color space and its legacy usage (lots of textures already exist as defined in RGB), it is a lot more convenient to input the values of textures as an RGB, convert them internally to the spectral domain and convert them back for the output image.
